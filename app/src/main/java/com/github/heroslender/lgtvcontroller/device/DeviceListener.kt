...
class DeviceListener(
    ...
    override fun onPairingRequired(
        ...
        Log.d("Device", "listDevices :::: onPairingRequired type: $pairingType")
        val status = if (pairingType == DeviceService.PairingType.PIN_CODE) {
            DeviceStatus.PIN_REQUIRED
        } else {
            DeviceStatus.PAIRING
        }
        manager.onDevicePairing(device, status)
